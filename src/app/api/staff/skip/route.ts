      ...
      return NextResponse.json(
        {
          ...
          message:
            "You can only SKIP the ticket your counter is currently serving.",
        },
        { status: 409 }
      );
    }

    await updateTicketStatus(ticket_id, "skipped");

    return NextResponse.json({ message: "ticket skipped" });
  } catch (error) {
    console.error("Staff skip error:", error);
    return NextResponse.json(
